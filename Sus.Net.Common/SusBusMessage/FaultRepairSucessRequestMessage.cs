...
namespace Sus.Net.Common.SusBusMessage
{
    ...
    /// 机修完成维修
    /// </summary>
    public class FaultRepairSucessRequestMessage : MessageBody
    {
        public FaultRepairSucessRequestMessage(byte[] resBytes) : base(resBytes)
        {
        }
        public static FaultRepairSucessRequestMessage isEqual(byte[] resBytes)
        {
            var rCmd = HexHelper.BytesToHexString(new byte[] { resBytes[2] });
            if (!SuspeConstants.cmd_Fault_Repair_Sucess_Repair.Equals(rCmd)) return null;

            var raddress = HexHelper.BytesToHexString(new byte[] { resBytes[4], resBytes[5] });
            if (!SuspeConstants.address_Fault_Repair_Sucess_Repair.Equals(raddress)) return null;
            IList<byte> bList = new List<byte>();
            for (var index = 7; index < 12; index++)
            {
                bList.Add(resBytes[index]);
            }
            return new FaultRepairSucessRequestMessage(resBytes)
            {
